...
public class Subber {
    ...
    public InetAddress getPeer(int futureIndex, String jid) throws UnknownHostException {
        while (true) {
            ConsumerRecords<String, String> records = this.get();
            for (ConsumerRecord<String, String> record : records) {
                String[] msgs = record.value().split(":");
                String prefix = msgs[0];
                if (!prefix.equals(Pubber.PUB_PREFIX))
                    continue;
                String jobid = msgs[1];
                if (!jid.equals(jobid))
                    continue;
                int index = Integer.parseInt(msgs[2]);
                if (index != futureIndex)
                    continue;
                return InetAddress.getByName(msgs[3]);
            }
        }
    }
}
