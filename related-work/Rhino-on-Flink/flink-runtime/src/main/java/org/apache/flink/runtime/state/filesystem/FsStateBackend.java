...
public class FsStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
	...
	@Override
	public Map<StateDescriptor, OperatorStatePartition> extractState() throws IOException, FlinkException {
		checkNotNull(this.keyedStateBackends);
		Map<StateDescriptor, OperatorStatePartition> states = new HashMap<>();

		for (Entry<StateDescriptor, KeyedStateBackend> keyedStateBackend : keyedStateBackends
				.entrySet()) {
			StateDescriptor info = keyedStateBackend.getKey();
			Map<String, OperatorStatePartition> operatorStates = keyedStateBackend.getValue().extractState();
			for (Entry<String, OperatorStatePartition> entry : operatorStates.entrySet()) {
				StateDescriptor newInfo = new StateDescriptor(info.jobID, info.jobVertexID, info.subtaskIndex, info.keyGroupRange);
				newInfo.setName(entry.getKey());
				states.put(newInfo, entry.getValue());
			}
		}
		return states;
	}
	...
	public void applyState(byte[] bytes) throws FlinkException, IOException {
		...
		for (int i = 0; i < nstate; i++) {
			StateDescriptor meta = new StateDescriptor(dd);
			String tableName=meta.name;
			int length = dd.readInt();
			byte[] buffer = new byte[length];
			dd.read(buffer);
			if (keyedStateBackends.containsKey(meta)) {
				KeyedStateBackend ksb = keyedStateBackends.get(meta);
				System.out.printf("apply state in table:%s\n",tableName);
				ksb.applyState(tableName,buffer);
			}
		}
