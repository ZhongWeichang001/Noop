...
public class HeapKeyedStateBackend<K> extends AbstractKeyedStateBackend<K> {
    ...
    @Override
    public Map<String, OperatorStatePartition> extractState() throws FlinkException, IOException {
        Map<String, OperatorStatePartition> states = new HashMap<>();
        for (Map.Entry<String, StateTable<K, ?, ?>> entry : registeredKVStates.entrySet()) {
            OperatorStatePartition s = entry.getValue().extractState();
            states.put(entry.getKey(), s);
            LOG.info("extract state:{} with {} bytes", entry.getKey(), s.toBytes().length);
        }
        return states;
    }
    ...
    public void applyState(String tableName, byte[] bytes) throws IOException {
