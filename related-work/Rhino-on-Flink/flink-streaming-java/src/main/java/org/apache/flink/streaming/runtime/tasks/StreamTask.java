...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
    ...
    public void broadcastBatchSignal(boolean rescale) {
        try {
            if (recordWriter instanceof NonRecordWriter) {
                ...
                String prefix = rescale ? Pubber.MIGRATION_PREFIX : Pubber.BATCH_PREFIX;
                LOG.info("{} in subtask {}, epoch {}", prefix, index, epoch++);
                pubber.send(topic, prefix + ":" + epoch + ":" + index);
                return;
            } else {
                EndOfBatchEvent signal = new EndOfBatchEvent();
                if (rescale) {
                    setNewRouting();
                    signal.setRescale();
                    doMigration();
                }
                recordWriter.getRecordWriter(0).broadcastEvent(signal, true);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void doMigration() throws IOException, FlinkException {
        int nstate = getStateDescriptor();
        if (nstate != 0) {
            try {
                LOG.info("changeKeyGroupRange with {} state", nstate);
                int parallelism = getEnvironment().getTaskInfo().getNumberOfParallelSubtasks();
                int futureIndex = (getEnvironment().getTaskInfo().getIndexOfThisSubtask() + 1) % parallelism;
                pubState(getSnapshot().toBytes());
                Thread.sleep(200);
                byte[] bytes = subState(futureIndex);
                LOG.info("receive bytes:{}", bytes.length);
    ...
    public BinaryState getSnapshot() throws IOException, FlinkException {
        if (stateBackend == null)
            return new BinaryState(new HashMap<>());
        return new BinaryState(stateBackend.extractState());
    }
    ...
    public void setNewRouting() throws Exception {
        ...
        if (nChannel != 1) {
            recordWriter.getRecordWriter(0).setRouting();
    ...
    void pubState(byte[] bytes) throws UnknownHostException, InterruptedException {
        LOG.info("pub state:{}", bytes.length);
        ...
        int port = Integer.valueOf(ProcessConf.getProperty(ProcessConf.MIGRATION_PORT))
                + subindex;
        String pubTopic = ProcessConf.getProperty(ProcessConf.SYNC_TOPIC);
        String pubPrefix = Pubber.PUB_PREFIX;
        String host = InetAddress.getLocalHost().getHostName();
        String pubMsg = String.format("%s:%s:%s:%s", pubPrefix, getEnvironment().getJobID().toString(),
                subindex, host);
        ...
        pubber.send(pubTopic, pubMsg);
    }

    byte[] subState(int futureIndex) throws IOException {
        Subber subber = new Subber();
        InetAddress where = subber.getPeer(futureIndex, getEnvironment().getJobID().toString());
        LOG.info("future index:{} should get from {}", futureIndex, where.toString());
        int port = Integer.valueOf(ProcessConf.getProperty(ProcessConf.MIGRATION_PORT));
        port += futureIndex;
        return fetchState(where, port);
    }

    byte[] fetchState(InetAddress where, int port) throws IOException {
