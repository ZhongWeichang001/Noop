...
public class ShardedRedisClusterClient extends BasicKVDatabaseClient{
	...
	private static synchronized void initProcessLevelClient() throws Exception {
		if (processLevelSingletonClient == null) {
			...
			String hostsString = pasaProperties.getProperty(ProcessLevelConf.REDIS_HOSTS_KEY, "localhost");
			int dbID = Integer.parseInt(pasaProperties.getProperty(ProcessLevelConf.REDIS_DB_INDEX_KEY, "0"));
			int poolSize = Integer.parseInt(pasaProperties.getProperty(ProcessLevelConf.REDIS_CONNECTION_POOL_SIZE_KEY, "4"));
